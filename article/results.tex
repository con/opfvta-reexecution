...
Complimentary, and initially possibly appearing contradictory, to the aforementioned advise we recommend to avoid building separate containers for related steps, in particular if supported by the same toolkits, and rather define and use different \emph{entry points} to the same container.
...
The need for an entire image rebuilding also means that assuring consistent functionality in view of frequent updates is more difficult inside containers than in an ad-hoc environment.
